...
On the first glance, it's still a close rase, however, there seems to be more of a differens in medians.
...
Let's take more in-depth look with infer.

```{r echo=TRUE}
observed_median_diff_usa_deu <- data %>% 
  filter(iso_code %in% c("USA", "DEU")) %>% 
  specify(new_cases_per_million ~ iso_code) %>% 
  calculate("diff in medians", order = c("DEU", "USA"))

observed_median_diff_usa_deu
...
null_dist_usa_deu %>% 
  visualise() +
  shade_p_value(observed_median_diff_usa_deu,
                direction = "two-sided")

p_value_median_usa_deu <- null_dist_usa_deu %>% 
  get_p_value(obs_stat = observed_median_diff_usa_deu,
              direction = "two-sided")
