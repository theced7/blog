...
Let's take more in-depth look with infer. Again, we start off by calculating the difference in means which we _observed_ in our data. 

Then we let infer _generate_ data which simulates a population where the null hypothesis is true. In this _generated_ data where we also calculate the difference between means. 
...
We see that the difference in means between both countries equals `r round(observed_mean_diff_usa_deu,2)`. 

Another very interesting feature of infer is it's ability to calculate [confidence intervals](https://en.wikipedia.org/wiki/Confidence_interval). 
