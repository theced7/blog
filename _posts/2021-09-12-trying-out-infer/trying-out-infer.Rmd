...
description: |
  Firstly, this blog post is ought to be an introduction to `infer`. As it turns out, this is also a great way of getting to grips with some basic statistical reasoning. So, if you're relatively new in the world of null hypotheses and boxplots, I believe this might also be a good learning opportunity. 
  
  The key question we are asking in this blog post is: TODO
author:
  - name: theCed7
...
First things first: Let's load [infer](https://infer.netlify.app/) and the rest of the tidyverse:
...
Infer is a package which aims to aid statistical testing in a "tidyverse way".

As such, we also need some data to conduct statistical testing on. As it is 2021, we might as well use something coronavirus-related. This download the vast _Our World in Data_ source file and reads it into R. I will also focus on the UK, since it's one of the countries with the most cohesive data available.
...
So, in a world where the number of new covid cases was independent of the country they appeared in, the possibility of observing a difference in means of cases by country of `r round(observed_mean_diff_usa_deu, 2)` (or more) would be `r p_value_mean_usa_deu`. 

<aside>
Note the warning we get if our p-value is calculated as 0. In reality, to get a probability of exactly 0 is highly unlikely. So our p-value would actually be more something along the lines of this: 0.000001. Because we used a rather "brute force" approach (by generating our null hypothesis data) these tiny nuances get lost and infer issues a warning.
</aside>

Appalled by this bulky wording? Understandable. However, this is due to the very definition of the **p-value**, one of the cornerstones of statistical analysis. And, since this is a very unintuitive value where mistakes are easily made, I believe it's important to get familiar with it from the beginning. 
